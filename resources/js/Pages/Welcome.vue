<template>
    <div>
        <div @click="zoom" class="p-4 ring-2 m-4 bg-red-300">button</div>
...
const drawDotsAndLines = (ctx, data) => {
  ...
  data.forEach((element, index) => {
    Object.entries(element).forEach(([id, { X, Y }]) => {
  ...
  Object.entries(data[data.length - 1]).forEach(([id, { X, Y }]) => { // Assumes the latest data is the last entry
    const scaledX = centerX + X * scale;
    const scaledY = centerY + Y * scale;
    const color = idToColor(id);
...
onMounted(() => {
  fetchCoordinates();
  intervalId = setInterval(fetchCoordinates, 1000); // Refresh every second
});
